"""
This code is modified by Rachana Jayaram from Hengyuan Hu's repository.
...
if __name__ == "__main__":
    ...
    for idx, emb_dim in enumerate(emb_dims):  # available embedding sizes
        ...
        weights[idx], word2emb = create_glove_embedding_init(
            label2ans, glove_file
        )
    np.save(
        "data/glove6b_emb_%dd.npy" % functools.reduce(operator.add, emb_dims),
        np.hstack(weights),
    )
