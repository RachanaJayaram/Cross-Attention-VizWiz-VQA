...
from model.graph.graph_att_layer import GraphSelfAttentionLayer
...
class GAttNet(nn.Module):
    def __init__(
        ...
        self.self_weights = MultiLayerNet(
            [in_feat_dim, out_feat_dim], dropout=dropout
        )
        self.bias = MultiLayerNet([label_num, 1], bias=label_bias)
    ...
    def forward(self, v_feat, adj_matrix, pos_emb=None):
        ...
        if self.pos_emb_dim > 0 and pos_emb is None:
            ...
            raise ValueError(
                f"position embedding is NOT None " f"with pos_emb_dim < 0"
            )
