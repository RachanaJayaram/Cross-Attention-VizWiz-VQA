...
class GraphSelfAttentionLayer(nn.Module):
    def __init__(
        ...
        if self.pos_emb_dim > 0:
            self.pair_pos_fc1 = MultiLayerNet(
                [pos_emb_dim, self.fc_dim], None, dropout[0]
            )
        self.query = MultiLayerNet([feat_dim, self.dim[0]], None, dropout[0])
        self.nongt_dim = nongt_dim

        self.key = MultiLayerNet([feat_dim, self.dim[1]], None, dropout[0])
    ...
    def forward(
        self, roi_feat, adj_matrix, position_embedding, label_biases_att
    ):
        """
        Args:
        ...
        if adj_matrix is not None:
            ...
            weighted_aff_masked = (
                weighted_aff_masked + label_biases_att.unsqueeze(3)
            )
